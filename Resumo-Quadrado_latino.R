...
# Agora vamos montar um data.frame com os dados e os indicadores de tratamentos, 
# inhas e colunas
trat<-c("B","A","D","E","C","C","B","E","A","D","A","D","B","C","E","D",
        "E","C","B","A","E","C","A","D","B")
trat<-as.factor(trat)


lat <- data.frame(linha=factor(rep(1:5, each=5)), col=factor(rep(1:5,5)), trat=trat, resp=x)
# Note que usamos a função factor para indicar que as variáveis linhas e colunas são níveis de 
# fatores e não valores numéricos.
...
lat
# Caso o arquivo esteja em outro diretório deve-se colocar o caminho completo deste diretório 
# no argumento de read.table acima.
...
# Nos gráficos e resultados acima procuramos captar os principais aspectos dos dados 
# bem como verificar se não há interação entre linhas, colunas e tratamentos, 
# o que não deve acontecer neste tipo de experimento.
# A seguir vamos ajustar o modelo e obter outros resultados, incluindo a análise de 
# resíduos e testes para verificar a validades dos pressupostos do modelo.

# ANOVA
lat.av <- aov(resp ~ col + linha + trat)
anova(lat.av)
names(lat.av)
# REPARAR SE OS GRAUS DE LIBERDADE ESTÁ OK
# REJEITAMOS HO QUANDO P-VALOR FOR MENOR QUE 0.05
...
# Teste para Normalidade dos ResC-duos
residuos <- (lat.av$residuals)
shapiro.test(residuos)
# Como foi detectado efeito de tratamentos faz-se um teste de comparações múltiplas e 
#  encerra-se as análises desanexando o objeto do caminho de procura
